...
An alternative mechanism could be formed whereby stars within the gap have magnetic activity so great that noise dominates their light curves making observation of their rotation impossible.
...
The gap would then represent a time where the average magnetic activity of stars would then be large enough that the noise permeates the entire magnetic activity cycle and no observations of rotation could be made.
Minima in the magnetic cycle photometric variability occur at times when the star is most active and photometrically noisy - complicating the detection of the rotation period through photometric variability.
Observations of rotation period should therefore be more likely occur when a star is minimally active.
Given that \rper{} decreases towards the gap, the maximum activity where rotation remains detectable must also decrease.
If this were the case then the decrease in photometric variability should be coincident with a decrease in the scatter of the photometric variability of stars in a given colour and rotational period range.
This is not reflected in the data - as seen in Figure \ref{fig:colour_rper}- the scatter on \rper{} of stars does not decrease towards the gap suggesting that the gap does not arise from this proposed selection effect.

\citet{reinhold_spot_2018} propose an explanation for the gap in which the amplitude decrease of \rper{} can be explained towards the gap if we assume that stars are spot-dominated below the gap, undergo a period of spot-faculae cancellation before returning to spot-dominance above the gap.
...
If the gap is indeed caused by the transition from spot to faculae dominance then the decrease in \rper{} would not be met with a decrease in the scatter on \rper{} in a slice of colour and rotation period.
They propose that the transition from spot to faculae dominance is representative of a transition to small, short-lived spots and longer living faculae which introduce noise to the spectra, resulting in low detectability of rotation.

expression of stellar spots is the result of the expression of 
...
Another interesting quantity to consider here is the scatter of photometric variability of stars for a given slice of colour and rotation period.
While large-scale trends of the photometric variability of stars have been shown to be a function of the colour and the rotation period, two stars of the same colour and rotation period and colour can be observed at different points in their magnetic activity cycle.
The magnetic activity cycle can be broken into two distinct regimes - the spot and faculae-dominated regimes.
Within the spot-dominated regime photometric variability, on a star on a magnetic activity time scale, "high activity" regimes are characterised by decreases to the observed brightness of a star.
In the faculae-dominated regime, the opposite is true: high activity regimes are characterised by increases to the observed brightness of a star.
Independent of the regime the feature by which rotation is observed tends to be the stellar spots \citep{reinhold_2021}.
Therefore, at times of low activity, when fewer surface spots are produced, the photometric variability of a star on a rotational timescale decreases.
Further, rotation tends to be less readily detectable at times of high activity when the light curve is noisy from the stochastic production of a larger number of surface features.
The detectability of rotation of a star is therefore a function of time throughout a magnetic activity timescale and \rper{} of a star of otherwise the same stellar parameters will vary depending on when it is observed in its magnetic activity cycle.
This is reflected when comparing \rper{} of stars with similar colour and rotation period as shown in Figure \ref{fig:colour_rper}.
This quantity is of interest here because the scatter \rper{} will vary with different explanations for the intermediate period gap.
